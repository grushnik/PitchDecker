...
def add_left_bullets_vert_center(slide, prs, lines, *, font_pt=28,
...
def add_center_paragraph(slide, prs, text, *, font_pt=30,
                         left_in=0.9, right_margin_in=0.9,
                         top_in=2.0, bottom_in=6.5, uplift_ratio=0.20,
                         wrap_chars=20):
...
downsides_items = make_editor(
    ...
    initial_rows=[{"Bullet": "The cost of plasma-based fertilizers compared to synthetic fertilizers.", "Med (3)": True}],
...
def build_ppt(payload):
    ...
    add_center_paragraph(
        ...
        uplift_ratio=0.20, wrap_chars=20
    )

    # 5) Radom team (image left, short paragraph right)
    ...
    scale2 = min(5.0 / img2_w_in, max_h / img2_h_in)
    ...
    right_text_top = y_center2 - right_text_height / 2.0

    team_text = ("Diverse, resourceful, motivated team, battle-hardened by 31 years of "
                 "combined entrepreneurial experience.")
    tb = s5.shapes.add_textbox(Inches(right_text_left), Inches(right_text_top),
                               Inches(right_text_width), Inches(right_text_height))
    tf = tb.text_frame; tf.clear()
    p = tf.paragraphs[0]
    r = p.add_run(); r.text = team_text; r.font.size = Pt(24); r.font.color.rgb = RGBColor(0,0,0)
    p.alignment = PP_ALIGN.RIGHT
    ...
    add_left_bullets_vert_center(
        ...
        left_in=0.9, right_margin_in=0.9, top_in=2.0, bottom_in=6.5,
        uplift_ratio=0.20, wrap_chars=40
    )
    ...
    for i, line in enumerate(chunks):
        ...
        run.font.size = Pt(24); run.font.color.rgb = RGBColor(0,0,0)
